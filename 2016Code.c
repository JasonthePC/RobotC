...
#pragma config(Motor,  port1,           rightForward,  tmotorVex393_HBridge, openLoop, encoderPort, I2C_5)
#pragma config(Motor,  port2,           rareLaunch,    tmotorVex393_MC29, openLoop, driveRight, encoderPort, I2C_1)
#pragma config(Motor,  port3,           pepesLaunch,   tmotorVex393_MC29, openLoop, reversed, driveLeft, encoderPort, I2C_2)
...
#pragma config(Motor,  port10,          leftForward,   tmotorVex393_HBridge, openLoop, reversed, encoderPort, I2C_6)
...
float signSqrt(float input){
	return sgn(input) * sqrt(abs(input));
}

float signSquare(float input){
	return sgn(input) * pow(input,2);
}
...
task driverInputHandler_ARCADE(){
	float throttle2 = signSquare(vexRT(Ch2));
	float yaw2 = signSquare(vexRT(Ch1));
	int left = (int)floor(signSqrt(throttle2+yaw2));
	int right = (int)floor(signSqrt(throttle2-yaw2));
	setMotor(rightForward,right);
	setMotor(rightRear,right);
	setMotor(leftForward,left);
	setMotor(leftRear,left);
}
